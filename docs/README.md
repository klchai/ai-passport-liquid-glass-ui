...
| Wi-Fi | Optional saved-credential STA for SNTP time sync | `main/time_sync.c` | Credentials must be provisioned separately; connection, antenna, and RF performance require device measurements |
| Bluetooth LE | Always-on connectable NimBLE usage GATT server as `FoloPassport` | `main/usage_link.c` | A companion Mac writes usage payloads; radio range, coexistence, and power draw require device measurements |
