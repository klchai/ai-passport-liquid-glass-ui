...
| Wi-Fi | 使用已保存凭证的可选 STA，用于 SNTP 校时 | `main/time_sync.c` | 凭证需由其它流程预先写入；连接、天线与射频表现需真机测量 |
| Bluetooth LE | 常驻、可连接的 NimBLE 用量 GATT server，设备名为 `FoloPassport` | `main/usage_link.c` | 配套 Mac 推送用量 payload；射频范围、共存与功耗需真机测量 |
...
应用也可以使用 ESP-IDF 提供的定时器、FreeRTOS 任务和内部 Flash/NVS；番茄钟分支提供了 NVS 示例。Wi-Fi 和 Bluetooth LE 仍是 ESP-IDF 应用服务而非 BSP API：`time_sync.c` 负责使用已保存凭证启动可选 STA 与 SNTP，`usage_link.c` 负责与页面导航无关的常驻 GATT 服务。`demo/claude-buddy-port` 仍是更完整的 BLE 应用架构参考，不能替代对当前板卡天线、射频表现、功耗和共存行为的实测。当前产品与固件基线使用 8 MB Flash，包含 3 MB factory-app 分区，并固定保留设备身份与永久 Recovery 区域，使二创固件仍可通过小程序安装。
